        ...
        <div class="row g-5">
          ...
          <div class="col-lg-6">
            ...
            <div class="mb-4">
              ...
              <!-- For rentable items the price IS the daily rate (matches the app + backend) -->
              <div class="text-muted mt-1" style="font-size:.85rem;" v-if="isRentable">
                <i class="bi bi-clock me-1"></i>{{ formatPrice(product.price) }} / day for rental
              </div>
            </div>
            ...
            <div v-if="isRentable" class="card p-3 mt-3" style="border-radius:1rem; border-color:rgba(201,169,110,.25)!important;">
              ...
              <div class="row g-2 mb-3">
                <div class="col-6">
                  ...
                  <input type="date" class="form-control form-control-sm" v-model="rental.start_date" :min="minStart" />
                </div>
                <div class="col-6">
                  <label class="form-label">End Date</label>
                  <input type="date" class="form-control form-control-sm" v-model="rental.end_date" :min="rental.start_date || minStart" />
                </div>
              </div>
              <div class="mb-3">
                <label class="form-label">Payment</label>
                <select class="form-select form-select-sm" v-model="rentalPayment">
                  <option value="wallet">Wallet (held until return)</option>
                  <option value="cash">Cash on delivery</option>
                </select>
              </div>
              <div class="d-flex align-items-center justify-content-between mb-3" v-if="rentalTotal">
                <span class="text-muted">{{ formatPrice(product.price) }} × {{ rentalDays }} days</span>
                <span class="text-gold fw-700">{{ formatPrice(rentalTotal) }}</span>
              </div>
              <button class="btn btn-outline-gold w-100" @click="addRental" :disabled="rentalDays < 1 || rentalLoading">
                <span class="spinner-border spinner-border-sm me-2" v-if="rentalLoading"></span>
                <i class="bi bi-calendar-check me-2" v-else></i>Request Rental
              </button>
            </div>
          </div>
        </div>
...
// Earliest rental start is 3 days out, leaving time to deliver the item to the renter.
const minStart = new Date(Date.now() + 3 * 86400000).toISOString().split('T')[0]
const rentalPayment = ref('wallet')
...
// Whole-day gap between the two dates. The backend bills inclusively
// (days = diff + 1), so the displayed total matches what is charged.
const rentalDayDiff = computed(() => {
  if (!rental.value.start_date || !rental.value.end_date) return 0
  return Math.round((new Date(rental.value.end_date) - new Date(rental.value.start_date)) / 86400000)
})
const rentalDays = computed(() => (rentalDayDiff.value > 0 ? rentalDayDiff.value + 1 : 0))
const rentalTotal = computed(() => rentalDays.value * (product.value?.price || 0))
...
async function addRental() {
  ...
  try {
    // Rentals have their own escrow flow on the backend (POST /rentals),
    // separate from the order/cart path — create it directly, like the app.
    await rentalService.create({
      product_id: product.value.id,
      renter_id: auth.user.id,
      start_date: rental.value.start_date,
      end_date: rental.value.end_date,
      daily_price: product.value.price,
      total_days: rentalDays.value,
      total_price: rentalTotal.value,
      payment_method: rentalPayment.value,
    })
    toast.success('Rental requested — the owner will confirm it.')
    rental.value = { start_date: '', end_date: '' }
    router.push('/rentals')
  } catch (e) {
    toast.error(e.response?.data?.message || 'Could not create the rental')
  } finally {
